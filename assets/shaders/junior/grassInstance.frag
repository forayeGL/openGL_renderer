...
uniform sampler2D opacityMaskSampler;//透明蒙版
uniform sampler2D cloudMaskSampler;//透明蒙版
...
void main()
{
  ...
  float alpha =  texture(opacityMaskSampler, uv).r;
	...
	vec3 grassColor = result + ambient;

	vec3 windDirN = normalize(windDirection);
	vec2 cloudUV = worldXZ / cloudUVScale;
	cloudUV = cloudUV + time * cloudSpeed * windDirN.xz;
    float cloudMaskV = texture(cloudMaskSampler, cloudUV).r;
	vec3 cloudColor = mix(cloudBlackColor, cloudWhiteColor, cloudMaskV);
